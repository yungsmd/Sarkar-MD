      ...
      for (const jid of participants) {
         ...
         if (action == "add" && config.WELCOME ) {
           const userName = jid.split("@")[0];
                    const joinTime = moment.tz('Asia/Pakistan').format('HH:mm:ss');
                    const joinDate = moment.tz('Asia/Pakistan').format('DD/MM/YYYY');
...
"`, contextInfo: {
                  ...
                  externalAdReply: {
                     ...
                     sourceUrl: 'texhbybandali.blogspot.com'
                  }
               }
            })
         } else if (action == "remove" && config.WELCOME ) {
           const userName = jid.split('@')[0];
                    const leaveTime = moment.tz('Asia/pakistan').format('HH:mm:ss');
                    const leaveDate = moment.tz('Asia/Pakistan').format('DD/MM/YYYY');
                    const membersCount = metadata.participants.length;
            sock.sendMessage(id, {
