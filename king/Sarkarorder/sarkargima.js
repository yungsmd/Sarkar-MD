...
const query = m.body.slice(prefix.length + cmd.length).trim();
  ...
  if (validCommands.includes(cmd)) {
    ...
    try {
      await sock.sendMessage(m.from, { text: '*please wait Sarkar-MD is downlaoding ${*query}' });

      const images = [];

      for (let i = 0; i < numberOfImages; i++) {
        const endpoint = `https://api.nexoracle.com/search/google-image?apikey=sarkar_786&q=${encodeURIComponent(query)}`;
        ...
        if (response.status === 200) {
          ...
          throw new Error('Image downloading failed');
        }
      }
