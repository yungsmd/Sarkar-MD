...
const facebookCommand = async (m, Matrix) => {
  ...
  if (validCommands.includes(cmd)) {
    ...
    try {
      ...
      // Get video title for caption, or fallback to description/tags

      let videoCaption = fbData.title || fbData.description || '';

      if (!videoCaption && fbData.tags) {

        videoCaption = fbData.tags.join(' '); // Join tags if title or description is missing

      }

      if (!videoCaption) {

        videoCaption = 'No description available'; // Fallback if both are missing

      }

      console.log("Video caption used:", videoCaption); // Debugging caption
      ...
      if (fileSizeInMB <= 300) {
        ...
        const content = { 
          ...
          caption: `${videoCaption} - DOWNLOADED BY SARKAR-MD`, // Use video title or fallback
