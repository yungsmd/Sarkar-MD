...
const downloadTikTokVideo = async (m, gss) => {
  ...
  if (validCommands.includes(cmd)) {
    ...
    // API endpoint to fetch video with HD version

    const apiUrl = `https://www.dark-yasiya-api.site/download/tiktok?url=${encodeURIComponent(url)}`;
    ...
    try {
      ...
      const videoUrl = data?.result?.hdVideo || data?.result?.wmVideo || null;
