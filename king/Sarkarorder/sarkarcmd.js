...
const alive = async (m, sock) => {
  ...
  if (cmd === "menu") {
    ...
    const realTime = currentTime.format("HH:mm:ss");
    const xdate = currentTime.format("DD/MM/YYYY");

    let pushwish = "";
    if (currentTime.isBefore(moment().tz("Asia/Colombo").set({ hour: 5, minute: 0, second: 0 }))) {
      pushwish = `Good Morning 🌄`;
    } else if (currentTime.isBefore(moment().tz("Asia/Colombo").set({ hour: 11, minute: 0, second: 0 }))) {
      pushwish = `Good Morning 🌄`;
    } else if (currentTime.isBefore(moment().tz("Asia/Colombo").set({ hour: 15, minute: 0, second: 0 }))) {
      pushwish = `Good Afternoon 🌅`;
    } else if (currentTime.isBefore(moment().tz("Asia/Colombo").set({ hour: 18, minute: 0, second: 0 }))) {
...
...`; // Shortened for readability
    ...
    const content = generateWAMessageFromContent(
      m.key.remoteJid,
      proto.Message.fromObject({ conversation: aliveMessage }),
      { quoted: m }
    );
    await sock.relayMessage(m.key.remoteJid, content.message, { messageId: content.key.id });
  }
};
